...
const Title2=()=><span style={{fontSize:20,fontWeight:600,letterSpacing:3}}>案件登记</span>;
...
class Modal2 extends React.Component {
    ...
    componentWillReceiveProps(props){
        ...
        if(props.showFlag){ //console.log('componentWillReceiveProps',props.spanTitleEle);
            // 模态框弹起是的信息自动填充；            
            var jsonOld=JSON.parse(props.spanTitleEle.getAttribute('config'));console.log(11111,jsonOld);
            if(jsonOld!=null){
                console.log(2222222);
                ...
                console.log(33333333,'huang');
                // 因为modal处于隐藏状态，所以他的元素对象找不到；
                // $('.attr td .checkbox').each((k,v)=>{
                //     console.log(v);
                //     v.checked='checked';
                // });
                // $('.attr td input[type="text"]').val('');
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td className='label'  colSpan='2' style={{display:this.state.params.label?'block':'none',padding:'10px 0px'}}>
                                ...
                                {this.state.params.label?this.showModal():''}
                            </td>
                        </tr>								
                        <tr>
            ...
            <Row>
                {
                    this.params.map((item,key)=>{
                        return (
                            <Col className='col' key={key} span={item} onDragOver={this.dragOver} onDrop={(e)=>{this.dragDrop(e,key)}} >{this.state.comps[key]}</Col>
                        )
                    })
                }
...
class Drag extends Component{
    constructor(){
        ...
        this.CompParams={
            ...
            textarea:{params:{Comp:<Textarea2 />,className:'component textarea',text:'详细描述    :'},spanTitleStyle:{verticalAlign:'top'},dragStart:()=>{this.dragStart(this.CompParams.textarea.params,'Comp')}},
